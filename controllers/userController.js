...
const getSignedUrl = async (profileImageUrl) => {
  ...
  // If it's a local URL, return it as-is (for local development)
  if (profileImageUrl.startsWith('/uploads/')) {
    console.log(`🔗 Returning local URL: ${profileImageUrl}`);
    return profileImageUrl;
  }
  
  // Only try GCS signed URL if bucket is available and it's a GCS file
  if (bucket) {
    try {
      ...
      console.log(`🔗 Generated GCS signed URL for: ${fileName}`);
      return url;
    } catch (err) {
  ...
  console.log(`🔗 Using original URL (no GCS): ${profileImageUrl}`);
  return profileImageUrl;
};
...
exports.createUser = async (req, res) => {
  ...
  console.log('👤 Creating user with data:', { name, userId, phone, email, role });
  ...
  try {
    ...
    if (req.file) {
      ...
      console.log(`✅ Profile image URL set to: ${profileImageUrl}`); 
    } else {
    ...
    console.log(`✅ User created with profile image: ${user.profileImageUrl}`);
