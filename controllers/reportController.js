...
exports.getDailyAttendance = async (req, res) => {
  ...
  try {
    ...
    overtimeRecords.forEach((ot) => {
      ...
      if (
        ...
        if (process.env.NODE_ENV !== "production") {
          console.log(
            `📊 Overtime mapping: userId=${userId}, date=${dateStr}, hours=${ot.hours}, status=${ot.status}, key=${dateKey}`,
          );
        }
      }
    });
    ...
    attendanceRecords.forEach((record) => {
      const userId =
        record.user?._id?.toString() || record.user?.toString() || record.user;
      ...
      for (const date of dates) {
        ...
        if (existingRecord) {
          ...
          const normalizedCheckInLocation = existingRecord.checkInLocation
            ? {
                longitude:
                  existingRecord.checkInLocation.longitude ||
                  existingRecord.checkInLocation.lng ||
                  null,
                latitude:
                  existingRecord.checkInLocation.latitude ||
                  existingRecord.checkInLocation.lat ||
                  null,
          ...
          const normalizedCheckOutLocation = existingRecord.checkOutLocation
            ? {
                longitude:
                  existingRecord.checkOutLocation.longitude ||
                  existingRecord.checkOutLocation.lng ||
                  null,
                latitude:
                  existingRecord.checkOutLocation.latitude ||
                  existingRecord.checkOutLocation.lat ||
                  null,
...
exports.getMonthlySummary = async (req, res) => {
  ...
  try {
    ...
    const summaryWithOvertime = summary.map((record) => {
      ...
      const recalculatedAbsentDays = Math.max(
        0,
        totalWorkingDaysCount - actualPresentDays - leaveDays,
      );
    ...
    const usersWithOvertimeOnly = Array.from(overtimeMap.keys())
      ...
      .map((userId) => {
        ...
        const userRecord = overtimeRecords.find(
          (ot) => (ot.user?.toString() || ot.user) === userId,
        );
        if (userRecord) {
